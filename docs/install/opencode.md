...
# Installing shepherd-pr on OpenCode

## What gets emitted

- `package.json` (shared with the pi adapter when both are active)
- `.opencode/plugins/shepherd-pr.js`, the OpenCode plugin module that registers the plugin's skills directory
...
```json
{ "plugin": ["shepherd-pr@git+https://github.com/prime-radiant-inc/shepherd-pr.git"] }
```
